...
const TOPIC_POOLS = [
  [
    `${CURRENT_YEAR}년 청년 주택드림 청약 통장 조건 및 1순위 신청 가이드`,
    `스마트스토어 초보 위탁판매자가 반드시 알아야 할 마진 계산 공식`,
    `${CURRENT_YEAR} 직장인 주말 N잡 부업 추천 TOP 5 (시간 대비 고수익)`,
    `제주도 3박 4일 렌터카 여행 힐링 코스 및 숨은 로컬 맛집`,
  ],
  [
    `${CURRENT_YEAR}년 연말정산 환급금 극대화 절세 꿀팁 & 소득공제 체크리스트`,
    `AI 챗GPT 업무 활용법: 보고서 작성 시간을 반으로 줄이는 프롬프트 10선`,
    `일본 도쿄 자유여행 가성비 호텔 & 지하철 패스 총정리`,
    `초기 비용 없이 시작하는 디지털 파일(PDF 전자책) 판매 부업 노하우`,
  ],
  [
    `${CURRENT_YEAR} 하반기 부동산 청약 시장 전망 및 무주택자 내집마련 전략`,
    `다이어트 정체기 극복하는 직장인 간헐적 단식 식단 가이드`,
    `노션(Notion) 생산성 200% 올리는 일정 & 프로젝트 관리 템플릿`,
    `강릉 1박 2일 바다 여행 필수 드라이브 코스 및 카페 투어`,
  ]
];
...
export default function Step1Research({ state, updateState, nextStep }) {
  ...
  const [topicPoolIdx, setTopicPoolIdx] = useState(0);

  const currentTopics = TOPIC_POOLS[topicPoolIdx];

  const handleShuffleTopics = () => {
    setTopicPoolIdx((prev) => (prev + 1) % TOPIC_POOLS.length);
  };
        ...
        <div className="bg-slate-50/80 p-3.5 rounded-xl border border-slate-200/70 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 text-[11px] font-bold text-slate-500 hover:text-emerald-700 bg-white px-2.5 py-1 rounded-lg border border-slate-200 shadow-2xs hover:bg-slate-50 transition-colors cursor-pointer"
            >
              <RotateCw className="w-3 h-3 text-emerald-600" />
              <span>다른 추천 주제 보기</span>
            </button>
          </div>
